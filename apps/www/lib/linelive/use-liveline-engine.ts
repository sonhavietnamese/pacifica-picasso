...
import { drawUserLines, updateAndDrawCrossParticles, type CrossParticle } from './draw/user-lines'
...
interface EngineConfig {
  ...
  paused?: boolean
  emptyText?: string
  priceLine?: { enable?: boolean; direction?: 'vertical' | 'horizontal' }
  futureSpace?: number

  // Drawing (future area)
  draw?: DrawConfig
  drawLines?: DrawLine[]
  onDrawEnd?: (line: DrawLine) => void
  onCrossing?: (lineIndex: number, crossCount: number) => void
...
function updateWindowTransition(
  ...
  if (wt.to !== cfg.windowSecs) {
    ...
    const fs = Math.min(cfg.futureSpace ?? 0, 0.99)
    const fe = fs > 0 ? (cfg.windowSecs * fs) / (1 - fs) : 0
    const targetRightEdge = now + cfg.windowSecs * buffer + fe
    const targetLeftEdge = targetRightEdge - cfg.windowSecs - fe
...
function updateBadgeDOM(
  ...
  badge.path.setAttribute(
    'd',
    cfg.badgeTail ? badgeSvgPath(pillW, pillH, BADGE_TAIL_LEN, BADGE_TAIL_SPREAD) : badgePillOnly(pillW, pillH)
  )
  ...
  const badgeLeft = liveDotX != null ? liveDotX + 8 - BADGE_PAD_X - tailLen : w - pad.right + 8 - BADGE_PAD_X - tailLen
...
function updateCandleWindowTransition(
  ...
  if (wt.to !== targetWindowSecs) {
    ...
    wt.rangeFromMin = displayMin
    wt.rangeFromMax = displayMax
    const cfs = Math.min(futureSpace, 0.99)
    const cfe = cfs > 0 ? (targetWindowSecs * cfs) / (1 - cfs) : 0
    const targetRightEdge = now + targetWindowSecs * buffer + cfe
    const targetLeftEdge = targetRightEdge - targetWindowSecs - cfe
...
/**
 * 2D segment intersection test. Returns the intersection point [x, y] or null.
 * Works for any orientation — handles circles, loops, and non-monotonic paths.
 */
...
const CROSS_SHAKE_DECAY = 0.002
const CROSS_SHAKE_MIN = 0.2
...
export function useLivelineEngine(
  ...
  // Drawing state (freehand pencil paths in the future area)
  const layoutRef = useRef<ChartLayout | null>(null)
  const nowXRef = useRef(0)
  const isDrawingRef = useRef(false)
  const drawPathRef = useRef<{ time: number; value: number }[]>([])

  // Crossing detection state (price vs drawn lines — segment intersection)
  const crossShakeRef = useRef({ amplitude: 0 })
  const crossParticlesRef = useRef<CrossParticle[]>([])
  const drawCrossCountsRef = useRef<number[]>([])
  const prevDrawNowRef = useRef(0)
  const prevDrawSmoothRef = useRef(0)
  const currentNowRef = useRef(0)
  ...
  const lastMultiSeriesRef = useRef<
    Array<{ id: string; data: LivelinePoint[]; value: number; palette: LivelinePalette; label?: string }>
  >([])
  ...
  // Mouse + touch events for hover/scrub + freehand drawing
  useEffect(() => {
    const container = containerRef.current
    if (!container) return
    ...
    const screenToTime = (x: number): number => {
      ...
      if (!layout) return 0
      return layout.leftEdge + ((x - layout.pad.left) / layout.chartW) * (layout.rightEdge - layout.leftEdge)
    }
    const screenToValue = (y: number): number => {
      const layout = layoutRef.current
      if (!layout) return 0
      return layout.minVal + (1 - (y - layout.pad.top) / layout.chartH) * layout.valRange
    }

    const MIN_POINT_DIST_SQ = 4 // 2px minimum between path points

    const addPathPoint = (x: number, y: number) => {
      const pt = { time: screenToTime(x), value: screenToValue(y) }
      const path = drawPathRef.current
      if (path.length > 0) {
        const last = path[path.length - 1]
        const layout = layoutRef.current
        if (layout) {
          const dx = layout.toX(pt.time) - layout.toX(last.time)
          const dy = layout.toY(pt.value) - layout.toY(last.value)
    ...
    const startDrawing = (x: number, y: number) => {
      ...
      let startPt: { time: number; value: number }
      if (Math.abs(x - nX) < snapThreshold) {
        startPt = { time: screenToTime(nX), value: configRef.current.value }
      } else {
        startPt = { time: screenToTime(x), value: screenToValue(y) }
      }
      drawPathRef.current = [startPt]
    }

    const onMouseDown = (e: MouseEvent) => {
      if (!(e.target instanceof HTMLCanvasElement)) return
      if (!configRef.current.draw?.enabled) return
      if (!layoutRef.current) return
      const rect = container.getBoundingClientRect()
      const x = e.clientX - rect.left
      const y = e.clientY - rect.top
      if (x <= nowXRef.current) return
      startDrawing(x, y)
    }
    ...
    const onMove = (e: MouseEvent) => {
      if (isDrawingRef.current) {
        const rect = container.getBoundingClientRect()
        addPathPoint(e.clientX - rect.left, e.clientY - rect.top)
        return
      }
    ...
    const onTouchStart = (e: TouchEvent) => {
      ...
      if (
        configRef.current.draw?.enabled &&
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (isDrawingRef.current) {
        const rect = container.getBoundingClientRect()
        addPathPoint(e.touches[0].clientX - rect.left, e.touches[0].clientY - rect.top)
        return
      }
      if (!configRef.current.scrub) return
      const rect = container.getBoundingClientRect()
  ...
  const draw = useCallback(() => {
    ...
    const cfg = configRef.current
    const priceLineDir: 'horizontal' | 'vertical' | 'none' =
      cfg.priceLine?.enable === false ? 'none' : (cfg.priceLine?.direction ?? 'horizontal')
    const dpr = getDpr()
    ...
    chartRevealRef.current = noMotion
      ? revealTarget
      : lerp(chartRevealRef.current, revealTarget, revealTarget === 1 ? CHART_REVEAL_SPEED_FWD : CHART_REVEAL_SPEED, dt)
    ...
    if (!hasData && !useStash && !useMultiStash) {
      ...
      const loadingColor =
        isCandle || cfg.isMultiSeries || lastMultiSeriesRef.current.length > 0 ? cfg.palette.gridLabel : undefined
    ...
    // Cross shake — wraps entire chart frame when price crosses a drawn line
    const cs = crossShakeRef.current
    let crossShakeApplied = false
    if (!noMotion && cs.amplitude > CROSS_SHAKE_MIN) {
      ctx.save()
      ctx.translate((Math.random() - 0.5) * 2 * cs.amplitude, (Math.random() - 0.5) * 2 * cs.amplitude)
      crossShakeApplied = true
    }
    if (cs.amplitude > 0) {
      cs.amplitude *= Math.pow(CROSS_SHAKE_DECAY, dt / 1000)
      if (cs.amplitude < CROSS_SHAKE_MIN) cs.amplitude = 0
    }
    ...
    if (isCandle) {
      ...
      const now = hasData || chartReveal < 0.005 ? Date.now() / 1000 - timeDebtRef.current : frozenNowRef.current
      ...
      const candleFutureExt =
        (cfg.futureSpace ?? 0) > 0
          ? (windowSecs * Math.min(cfg.futureSpace!, 0.99)) / (1 - Math.min(cfg.futureSpace!, 0.99))
          : 0
      const rightEdge = now + windowSecs * candleBuffer + candleFutureExt
      const leftEdge = rightEdge - windowSecs - candleFutureExt
      ...
      nowXRef.current = layout.toX(now)
      currentNowRef.current = now
      currentSmoothRef.current = closeLineSmoothInitedRef.current ? closeLineSmoothRef.current : (rawLive?.close ?? 0)
      ...
      drawCandleFrame(ctx, layout, cfg.palette, {
        ...
        liveCandle: drawLive,
        closePriceCandle:
          closeLineSmoothInitedRef.current && rawLive ? { ...rawLive, close: closeLineSmoothRef.current } : rawLive,
      ...
      if (badgeRef.current) {
        if (lineModeProg > 0.5 && cfg.showBadge) {
          ...
          const candleBadgeDotX = (cfg.futureSpace ?? 0) > 0 ? layout.toX(now) : undefined
      ...
      if (transition.startMs > 0 && effectiveMultiSeries.length > 1) {
        const msFs = Math.min(cfg.futureSpace ?? 0, 0.99)
        const msFe = msFs > 0 ? (cfg.windowSecs * msFs) / (1 - msFs) : 0
        const targetRightEdge = now + cfg.windowSecs * buffer + msFe
        const targetLeftEdge = targetRightEdge - cfg.windowSecs - msFe
      ...
      const msFutureExt =
        (cfg.futureSpace ?? 0) > 0
          ? (windowSecs * Math.min(cfg.futureSpace!, 0.99)) / (1 - Math.min(cfg.futureSpace!, 0.99))
          : 0
      const rightEdge = now + windowSecs * buffer + msFutureExt
      const leftEdge = rightEdge - windowSecs - msFutureExt
      ...
      if (seriesEntries.length === 0) {
        ...
        ctx.restore()
        if (badgeRef.current) badgeRef.current.container.style.display = 'none'
        if (crossShakeApplied) ctx.restore()
        rafRef.current = requestAnimationFrame(draw)
        return
      }
      ...
      nowXRef.current = layout.toX(now)
      currentNowRef.current = now
      currentSmoothRef.current = smoothValues.get(firstSeries.id) ?? firstSeries.value
      ...
      const lineFutureExt =
        (cfg.futureSpace ?? 0) > 0
          ? (windowSecs * Math.min(cfg.futureSpace!, 0.99)) / (1 - Math.min(cfg.futureSpace!, 0.99))
          : 0
      const rightEdge = now + windowSecs * buffer + lineFutureExt
      const leftEdge = rightEdge - windowSecs - lineFutureExt
      ...
      nowXRef.current = layout.toX(now)
      currentNowRef.current = now
      currentSmoothRef.current = smoothValue
      ...
      const recentDelta =
        lookback > 0 ? Math.abs(visible[visible.length - 1].value - visible[visible.length - 1 - lookback].value) : 0
      ...
      if (badge) {
        const badgeDotX = (cfg.futureSpace ?? 0) > 0 ? layout.toX(now) : undefined
        badgeYRef.current = updateBadgeDOM(
          badge,
          cfg,
          smoothValue,
    ...
    // Draw user lines (shared across all modes — renders after all pipeline content)
    const drawLayout = layoutRef.current
    const drawNow = currentNowRef.current
    const drawSmooth = currentSmoothRef.current

    // Crossing detection — segment intersection (handles circles, loops, any shape)
    const drawLines = cfg.drawLines ?? []
    const prevDN = prevDrawNowRef.current
    const prevDS = prevDrawSmoothRef.current
    const counts = drawCrossCountsRef.current
    while (counts.length < drawLines.length) counts.push(0)
    if (counts.length > drawLines.length) counts.length = drawLines.length

    if (drawLayout && drawLines.length > 0 && drawNow > 0 && prevDN > 0 && drawNow - prevDN < 0.5) {
      for (let i = 0; i < drawLines.length; i++) {
        const pts = drawLines[i].points
        ...
        for (let j = 0; j < pts.length - 1; j++) {
          ...
          if (hit) {
            counts[i]++
            cfg.onCrossing?.(i, counts[i])

            const crossX = drawLayout.toX(hit[0])
            const crossY = drawLayout.toY(hit[1])
            const color = drawLines[i].stroke ?? cfg.draw?.stroke ?? cfg.palette.line

            crossShakeRef.current.amplitude = Math.max(crossShakeRef.current.amplitude, 6)

            const cps = crossParticlesRef.current
            for (let k = 0; k < 18; k++) {
              const angle = Math.random() * Math.PI * 2
              const speed = 50 + Math.random() * 90
              cps.push({
                x: crossX + (Math.random() - 0.5) * 12,
                y: crossY + (Math.random() - 0.5) * 6,
    ...
    prevDrawNowRef.current = drawNow
    prevDrawSmoothRef.current = drawSmooth
    const crossCounts: number[] = counts.slice()
    ...
    // Cross particles (rendered after user lines so they appear on top)
    if (crossParticlesRef.current.length > 0) {
      updateAndDrawCrossParticles(ctx, crossParticlesRef.current, dt)
    }

    // Restore cross shake canvas transform
    if (crossShakeApplied) ctx.restore()
