...
export interface DrawOptions {
  ...
  priceLineDir?: 'horizontal' | 'vertical' | 'none'
}
...
export function drawMultiFrame(ctx: CanvasRenderingContext2D, layout: ChartLayout, opts: MultiSeriesDrawOptions): void {
  ...
  const scrubX = opts.scrubAmount > 0.05 ? opts.hoverX : null
  const plDir = opts.priceLineDir ?? 'horizontal'
  const perSeriesDir = plDir === 'horizontal' ? ('horizontal' as const) : ('none' as const)
  ...
  // Vertical price line for multi-series — drawn once with primary palette
  if (plDir === 'vertical') {
    const liveTipX = layout.toX(opts.now)
    ctx.save()
    ctx.setLineDash([4, 4])
    ctx.strokeStyle = palette.dashLine
    ctx.lineWidth = 1
    ctx.globalAlpha = reveal * (1 - opts.scrubAmount * 0.2)
    ctx.beginPath()
    ctx.moveTo(liveTipX, layout.pad.top)
    ctx.lineTo(liveTipX, layout.h - layout.pad.bottom)
    ctx.stroke()
    ctx.setLineDash([])
    ctx.restore()
  }
...
export function drawCandleFrame(
  ...
  if (lp > 0.01 && opts.lineVisible.length >= 2) {
    ...
    linePts = drawLine(
      ...
      opts.now_ms,
      colorBlend,
      fullLineMode ? (opts.priceLineDir ?? 'horizontal') : 'none',
  ...
  const candlePlDir = opts.priceLineDir ?? 'horizontal'
  if (candlePlDir !== 'none' && closeSource && closeAlpha > 0.01) {
    // Candle-colored close line (fades out with lineModeProg)
    if (lp < 0.99) {
      ctx.save()
      ctx.globalAlpha = closeAlpha * (1 - lp)
      drawClosePrice(ctx, layout, palette, closeSource, opts.scrubAmount, opts.liveBullBlend, candlePlDir, opts.now)
      ctx.restore()
    }
    ...
    if (lp > 0.01 && !fullLineMode) {
      ...
      ctx.beginPath()
      if (candlePlDir === 'vertical') {
        const dashX = layout.toX(opts.now)
        ctx.moveTo(dashX, pad.top)
        ctx.lineTo(dashX, h - pad.bottom)
      } else {
