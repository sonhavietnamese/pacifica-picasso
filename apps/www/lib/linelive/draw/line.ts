...
export function drawLine(
  ...
  now_ms: number = 0,
  colorBlend: number = 1,
  priceLineDir: 'horizontal' | 'vertical' | 'none' = 'horizontal',
  fillScale: number = 1
) {
  ...
  if (priceLineDir !== 'none') {
    ...
    ctx.beginPath()
    if (priceLineDir === 'vertical') {
      const liveTipXPos = toX(now)
      const centerXPos = pad.left + chartW / 2
      const currentX = chartReveal < 1 ? centerXPos + (liveTipXPos - centerXPos) * chartReveal : liveTipXPos
